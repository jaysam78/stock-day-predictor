    ...
    for index, row in enumerate(
        ...
        with st.expander(
            ...
            if row_id is not None:
                ...
                if st.button(
                    f"🗑 Remove {row['ticker']}",
                    key=f"remove_prediction_{row_id}_{index}",
                    use_container_width=True
                ):
...
1. Open **Analyze**, **Scanner**, or **My Stocks**.
2. Analyze the stock.
3. Press **Track this prediction**.
4. The stock appears in **Prediction Tracker**.

### Remove one prediction

Open the tracked stock and press:

**🗑 Remove**

Only that prediction is deleted.
...
This removes every tracked prediction.
...
The tracker will compare the original prediction with:
...
It then calculates whether the directional prediction was correct.
"""
    )
